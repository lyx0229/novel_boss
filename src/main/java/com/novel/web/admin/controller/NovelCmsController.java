...
public class NovelCmsController {
	...
	// 处理文件上传
	@RequestMapping(value = "/novel_upload", method = RequestMethod.POST)
	public @ResponseBody String novel_upload(@RequestParam("file") MultipartFile file, HttpServletRequest request) {
		String fileName = file.getOriginalFilename();
		int id = Util.toInt(request.getParameter("id"));
		logger.info("id:" + id);
		Novel novel = novelCmsService.findNovelById(id);
		logger.info("上传的文件名为：" + fileName);
		// 获取文件的后缀名
		String suffixName = fileName.substring(fileName.lastIndexOf("."));
		logger.info("上传的后缀名为：" + suffixName);
		String filePath = config.NOVEL_IMG + novel.getId() + "\\";
		String imgurl = filePath + fileName;
		for(int i = 0; i < 2; i++){
			imgurl = imgurl.substring(imgurl.indexOf("\\")+1);
        }
		...
		try {
			file.transferTo(dest);
			novel.setNovel_imgurl(config.DOMAIN+imgurl);
			novelCmsService.saveAndUpdate(novel);
		} catch (Exception e) {
			return "failed";
		}
		return "success";
